...
class Image(XI_IMG):
    ...
    def get_image_data_raw(self):
        ...
        """
        output_length = (
            self.get_bytes_per_pixel() * self.width * self.height
            + self.padding_x * self.height
        )
    ...
    def get_image_data_numpy(self, invert_rgb_order=False):
        ...
        try:
            if (
                self.frm == XI_IMG_FORMAT["XI_MONO8"].value
                or self.frm == XI_IMG_FORMAT["XI_RAW8"].value
            ):
                ...
                self.frm == XI_IMG_FORMAT["XI_MONO16"].value
                or self.frm == XI_IMG_FORMAT["XI_RAW16"].value
            ):
                ...
                self.frm == XI_IMG_FORMAT["XI_RAW8X2"].value
                or self.frm == XI_IMG_FORMAT["XI_RAW8X3"].value
                or self.frm == XI_IMG_FORMAT["XI_RAW8X4"].value
            ):
                ...
                or self.frm == XI_IMG_FORMAT["XI_RAW16X3"].value
                or self.frm == XI_IMG_FORMAT["XI_RAW16X4"].value
            ):
                ...
                self.frm == XI_IMG_FORMAT["XI_RGB24"].value
                or self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value
            ):
                c_array = c_ubyte * self.get_channel_count() * self.width * self.height
                data = c_array()
                c_arr_ops.arrRGB(
                ...
                self.frm == XI_IMG_FORMAT["XI_RGB48"].value
                or self.frm == XI_IMG_FORMAT["XI_RGB16_PLANAR"].value
            ):
                c_array = c_ushort * self.get_channel_count() * self.width * self.height
                data = c_array()
    ...
    def get_channel_count(self):
        ...
        if (
            self.frm == XI_IMG_FORMAT["XI_MONO8"].value
            or self.frm == XI_IMG_FORMAT["XI_RAW8"].value
            or self.frm == XI_IMG_FORMAT["XI_MONO16"].value
            or self.frm == XI_IMG_FORMAT["XI_RAW16"].value
        ):
            return 1
        elif (
            self.frm == XI_IMG_FORMAT["XI_RAW8X2"].value
            or self.frm == XI_IMG_FORMAT["XI_RAW16X2"].value
        ):
            return 2
        elif (
            self.frm == XI_IMG_FORMAT["XI_RAW8X3"].value
            or self.frm == XI_IMG_FORMAT["XI_RAW16X3"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB24"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB48"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB16_PLANAR"].value
        ):
            return 3
            ...
            or self.frm == XI_IMG_FORMAT["XI_RAW16X4"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB32"].value
            or self.frm == XI_IMG_FORMAT["XI_RGB64"].value
        ):
            return 4
        else:
            raise Xi_error(108)  # "Data format not supported"
...
class Camera(object):
    ...
    def open_device_by(self, open_type, val):
        ...
        if not self.CAM_OPEN:
            ...
            stat = self.device.xiOpenDeviceBy(
                XI_OPEN_BY[open_type], buf, byref(self.handle)
            )
            if not stat == 0:
                raise Xi_error(stat)
            self.CAM_OPEN = True
    ...
    def get_image(self, image, timeout=5000):
        ...
        the data.
        """
        stat = self.device.xiGetImage(self.handle, DWORD(timeout), byref(image))
    ...
    def get_device_info_string(self, param):
        ...
        stat = self.device.xiGetDeviceInfoString(self.dev_id, prm, val, DWORD(val_len))
    ...
    def set_param(self, param, val):
        ...
        stat = self.device.xiSetParam(
            self.handle, prm, val, val_len, XI_PRM_TYPE[val_type]
        )
    ...
    def get_param(self, param, buffer_size=256):
        ...
        if val_type == "xiTypeString":
            ...
            val_type == "xiTypeInteger"
            or val_type == "xiTypeBoolean"
            or val_type == "xiTypeCommand"
        ):
            val_len = DWORD(4)
            val = pointer(c_int())
        elif val_type == "xiTypeEnum":
        ...
        stat = self.device.xiGetParam(
            self.handle, prm, val, byref(val_len), byref(XI_PRM_TYPE[val_type])
        )
        ...
        if (
            val_type == "xiTypeInteger"
            or val_type == "xiTypeFloat"
            or val_type == "xiTypeInteger64"
        ):
    ...
    def set_test_pattern_generator_selector(self, test_pattern_generator_selector):
        ...
        """
        self.set_param(
            "test_pattern_generator_selector", test_pattern_generator_selector
        )
    ...
    def set_acq_transport_data_commit_total_size(
        self, acq_transport_data_commit_total_size
    ):
        """
        Total number of bytes to be commit in one time on transport (all transport buffers together). Increasing can enhance transport capacity. E.g. on USBXI_PRM_ACQ_TRANSPORT_DATA_COMMIT_TOTAL_SIZE
        """
        self.set_param(
            "acq_transport_data_commit_total_size", acq_transport_data_commit_total_size
        )
    ...
    def set_device_unit_register_selector_name(
        self, device_unit_register_selector_name
    ):
        """
        Selects register of selected device unit by name.XI_PRM_DEVICE_UNIT_REGISTER_SELECTOR_NAME
        """
        self.set_param(
            "device_unit_register_selector_name", device_unit_register_selector_name
        )
    ...
    def set_device_unit_register_enum_value_index(
        self, device_unit_register_enum_value_index
    ):
        """
        Sets/gets register enumerated value index.XI_PRM_DEVICE_UNIT_REGISTER_ENUM_VALUE_INDEX
        """
        self.set_param(
            "device_unit_register_enum_value_index",
            device_unit_register_enum_value_index,
        )
