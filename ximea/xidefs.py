...
XI_TEST_PATTERN = {
    ...
    "XI_TESTPAT_GREY_VERT_RAMP_MOVING": c_uint(
        6
...
XI_BIN_SELECTOR = {
    "XI_BIN_SELECT_SENSOR": c_uint(
        0
    ),  # parameters for image sensor binning are selected
    "XI_BIN_SELECT_DEVICE_FPGA": c_uint(
        1
    ),  # parameters for device (camera) FPGA decimation are selected
    "XI_BIN_SELECT_HOST_CPU": c_uint(2),  # parameters for Host CPU binning are selected
}

# Selects binning mode; to be used with
XI_BIN_MODE = {
    "XI_BIN_MODE_SUM": c_uint(
        0
    ),  # The response from the combined pixels will be added, resulting in increased sensitivity.
    "XI_BIN_MODE_AVERAGE": c_uint(
        1
...
XI_DEC_SELECTOR = {
    "XI_DEC_SELECT_SENSOR": c_uint(
        0
    ),  # parameters for image sensor decimation are selected
    "XI_DEC_SELECT_DEVICE_FPGA": c_uint(
        1
    ),  # parameters for device (camera) FPGA decimation are selected
    "XI_DEC_SELECT_HOST_CPU": c_uint(
        2
    ),  # parameters for Host CPU decimation are selected
}
...
XI_IMG_FORMAT = {
    ...
    "XI_FRM_TRANSPORT_DATA": c_uint(
        7
    ),  # Data from transport layer (e.g. packed). Depends on data on the transport layer (see Note7)
    "XI_RGB48": c_uint(
        8
    ),  # RGB data format. [Blue low byte][Blue high byte][Green low][Green high][Red low][Red high] (see Note5)
    "XI_RGB64": c_uint(
        9
    ),  # RGBA data format. [Blue low byte][Blue high byte][Green low][Green high][Red low][Red high][0][0] (Note5)
    "XI_RGB16_PLANAR": c_uint(10),  # RGB16 planar data format
    "XI_RAW8X2": c_uint(
        11
    ),  # 8 bits per pixel raw data from sensor(2 components in a row). [ch1 pixel byte] [ch2 pixel byte] 8 bits raw data from 2 channels (e.g. high gain and low gain channels of sCMOS cameras)
    "XI_RAW8X4": c_uint(
        12
    ),  # 8 bits per pixel raw data from sensor(4 components in a row). 	[ch1 pixel byte [ch2 pixel byte] [ch3 pixel byte] [ch4 pixel byte] 8 bits raw data from 4 channels (e.g. sCMOS cameras)
    "XI_RAW16X2": c_uint(
        13
    ),  # 16 bits per pixel raw data from sensor(2 components in a row). 	[ch1 pixel byte low] [ch1 pixel byte high] [ch2 pixel byte low] [ch2 pixel byte high] 16 bits (depacked) raw data from 2 channels (e.g. high gain and low gain channels of sCMOS cameras)
    "XI_RAW16X4": c_uint(
        14
    ),  # 16 bits per pixel raw data from sensor(4 components in a row). 	[ch1 pixel byte low] [ch1 pixel byte high] [ch2 pixel byte low] [ch2 pixel byte high] [ch3 pixel byte low] [ch3 pixel byte high] [ch4 pixel byte low] [ch4 pixel byte high] 16 bits (depacked) raw data from 4 channels (e.g. sCMOS cameras)
    "XI_RAW32": c_uint(
        15
    ),  # 32 bits per pixel raw data from sensor in integer format (LSB first). 4 bytes (LSB first) pixel (depacked) raw data
    "XI_RAW32FLOAT": c_uint(
        16
    ),  # 32 bits per pixel raw data from sensor in single-precision floating point format. 4 bytes per pixel (depacked) raw data
    "XI_RAW8X3": c_uint(
        17
    ),  # 8 bits per pixel raw data from sensor(3 components in a row). 	[ch1 pixel byte] [ch2 pixel byte] [ch3 pixel byte] raw data from 3 channels (e.g. ToF cameras)
    "XI_RAW16X3": c_uint(
        18
    ),  # 16 bits per pixel raw data from sensor(3 components in a row). [ch1 pixel byte low] [ch1 pixel byte high] [ch2 pixel byte low] [ch2 pixel byte high] [ch3 pixel byte low] [ch3 pixel byte high] 16 bits (depacked) raw data from 3 channels (e.g. ToF cameras)
}
...
XI_IMAGE_SEQUENCE_TYPE = {
    ...
    "XI_IMAGE_SEQUENCE_TYPE_EXPOSURES": c_uint(
        1
    ),  # Image sequence off different exposures. E.g. for HDR.
}
...
XI_TRG_SOURCE = {
    "XI_TRG_OFF": c_uint(
        0
    ),  # Capture of next image is automatically started after previous.
    "XI_TRG_EDGE_RISING": c_uint(
        1
    ),  # Capture is started on rising edge of selected input.
    "XI_TRG_EDGE_FALLING": c_uint(
        2
    ),  # Capture is started on falling edge of selected input
    "XI_TRG_SOFTWARE": c_uint(3),  # Capture is started with software trigger.
    "XI_TRG_LEVEL_HIGH": c_uint(
        4
    ),  # Specifies that the trigger is considered valid as long as the level of the source signal is high.
    "XI_TRG_LEVEL_LOW": c_uint(
        5
...
XI_TRG_SELECTOR = {
    ...
    "XI_TRG_SEL_EXPOSURE_ACTIVE": c_uint(
        1
    ),  # Trigger controls the start and length of the exposure.
    "XI_TRG_SEL_FRAME_BURST_START": c_uint(
        2
    ),  # Trigger starts the capture of the bursts of frames in an acquisition.
    "XI_TRG_SEL_FRAME_BURST_ACTIVE": c_uint(
        3
    ),  # Trigger controls the duration of the capture of the bursts of frames in an acquisition.
    "XI_TRG_SEL_MULTIPLE_EXPOSURES": c_uint(
        4
    ),  # Trigger which when first trigger starts exposure and consequent pulses are gating exposure(active HI)
    "XI_TRG_SEL_EXPOSURE_START": c_uint(
        5
    ),  # Trigger controls the start of the exposure of one Frame.
    "XI_TRG_SEL_MULTI_SLOPE_PHASE_CHANGE": c_uint(
        6
    ),  # Trigger controls the multi slope phase in one Frame (phase0 -> phase1) or (phase1 -> phase2).
    "XI_TRG_SEL_ACQUISITION_START": c_uint(
        7
    ),  # Selects a trigger that starts the Acquisition.
}

# Trigger overlap modes
XI_TRG_OVERLAP = {
    "XI_TRG_OVERLAP_OFF": c_uint(0),  # No trigger overlap is permitted.
    "XI_TRG_OVERLAP_READ_OUT": c_uint(
        1
    ),  # Trigger is accepted immediately after the exposure period. (see Note1)
    "XI_TRG_OVERLAP_PREV_FRAME": c_uint(
        2
    ),  # Trigger is accepted (latched) at any time during the capture of the previous frame.
}

# structure containing information about acquisition timing modes
XI_ACQ_TIMING_MODE = {
    "XI_ACQ_TIMING_MODE_FREE_RUN": c_uint(
        0
    ),  # camera acquires images at a maximum possible framerate
    "XI_ACQ_TIMING_MODE_FRAME_RATE": c_uint(
        1
    ),  # Selects a mode when sensor frame acquisition frequency is set to parameter FRAMERATE
    "XI_ACQ_TIMING_MODE_FRAME_RATE_LIMIT": c_uint(
        2
    ),  # Selects a mode when sensor frame acquisition frequency is limited by parameter FRAMERATE
}

# Enumerator for data target modes
XI_TRANSPORT_DATA_TARGET_MODE = {
    "XI_TRANSPORT_DATA_TARGET_CPU_RAM": c_uint(
        0
    ),  # normal CPU memory buffer is used for image data
    "XI_TRANSPORT_DATA_TARGET_GPU_RAM": c_uint(
        1
    ),  # data is delivered straight to GPU memory using GPUDirect technology
    "XI_TRANSPORT_DATA_TARGET_UNIFIED": c_uint(
        2
    ),  # CUDA managed memory is used for image data.
    "XI_TRANSPORT_DATA_TARGET_ZEROCOPY": c_uint(
        3
    ),  # CUDA zerocopy memory is used for image data.
}
...
XI_GPI_MODE = {
    "XI_GPI_OFF": c_uint(
        0
    ),  # Input is not used for triggering, but can be used to get parameter GPI_LEVEL. This can be used to switch I/O line on some cameras to input mode.
...
XI_GPO_MODE = {
    ...
    "XI_GPO_FRAME_ACTIVE": c_uint(
        2
    ),  # Output is on while frame exposure,read,transfer.
    "XI_GPO_FRAME_ACTIVE_NEG": c_uint(
        3
    ),  # Output is off while frame exposure,read,transfer.
    ...
    "XI_GPO_FRAME_TRIGGER_WAIT": c_uint(
        6
    ),  # Output is on while camera is ready for trigger
    "XI_GPO_FRAME_TRIGGER_WAIT_NEG": c_uint(
        7
    ),  # Output is off while camera is ready for trigger.
    "XI_GPO_EXPOSURE_PULSE": c_uint(
        8
    ),  # Output is on short pulse at the beginning of frame exposure.
    "XI_GPO_EXPOSURE_PULSE_NEG": c_uint(
        9
    ),  # Output is off short pulse at the beginning of frame exposure.
    "XI_GPO_BUSY": c_uint(
        10
    ),  # Output is on when camera has received trigger until end of transfer
    "XI_GPO_BUSY_NEG": c_uint(
        11
    ),  # Output is off when camera has received trigger until end of transfer
    "XI_GPO_HIGH_IMPEDANCE": c_uint(
        12
    ),  # Associated pin is in high impedance (tri-stated) and can be driven externally. E.g. for triggering or reading status by GPI_LEVEL.
    "XI_GPO_FRAME_BUFFER_OVERFLOW": c_uint(13),  # Frame buffer overflow status.
    "XI_GPO_EXPOSURE_ACTIVE_FIRST_ROW": c_uint(
        14
    ),  # Output is on while the first row exposure.
    "XI_GPO_EXPOSURE_ACTIVE_FIRST_ROW_NEG": c_uint(
        15
    ),  # Output is off while the first row exposure.
    "XI_GPO_EXPOSURE_ACTIVE_ALL_ROWS": c_uint(
        16
    ),  # Output is on while all rows exposure together.
    "XI_GPO_EXPOSURE_ACTIVE_ALL_ROWS_NEG": c_uint(
        17
    ),  # Output is off while all rows exposure together.
...
XI_METADATA_SAMPLING_MODE = {
    ...
    "XI_METADATA_SAMPLING_POINT_EXPOSURE_START": c_uint(
        1
    ),  # Selects the sampling point to exposure start
    "XI_METADATA_SAMPLING_POINT_EXPOSURE_END": c_uint(
        2
    ),  # Selects the sampling point to exposure end
    "XI_METADATA_SAMPLING_POINT_EXPOSURE_START_END": c_uint(
        3
    ),  # Selects the sampling point to exposure start and end
    "XI_METADATA_SAMPLING_POINT_READOUT_END": c_uint(
        4
    ),  # Selects the sampling point to readout end
}
...
XI_LED_MODE = {
    ...
    "XI_LED_EXT_EVENT_ACTIVE": c_uint(
        2
    ),  # Set led to blink if external signal detected.
...
XI_COUNTER_SELECTOR = {
    "XI_CNT_SEL_TRANSPORT_SKIPPED_FRAMES": c_uint(
        0
    ),  # Number of skipped frames on transport layer (e.g. when image gets lost while transmission). Occur when capacity of transport channel does not allow to transfer all data.
    "XI_CNT_SEL_API_SKIPPED_FRAMES": c_uint(
        1
    ),  # Number of skipped frames on API layer. Occur when application does not process the images as quick as they are received from the camera.
    "XI_CNT_SEL_TRANSPORT_TRANSFERRED_FRAMES": c_uint(
        2
    ),  # Number of delivered buffers since last acquisition start.
    "XI_CNT_SEL_FRAME_MISSED_TRIGGER_DUETO_OVERLAP": c_uint(
        3
    ),  # Number of missed triggers overlapped with exposure or read-out stage of previous frame – see XI_PRM_TRG_OVERLAP. (see Note1)
    "XI_CNT_SEL_FRAME_MISSED_TRIGGER_DUETO_FRAME_BUFFER_OVR": c_uint(
        4
    ),  # Number of missed triggers due to frame buffer full. (see Note1)
    "XI_CNT_SEL_FRAME_BUFFER_OVERFLOW": c_uint(
        5
    ),  # Internal camera frame buffer memory (RAM) full events counter. It can be incremented multiple times per one frame. (see Note1)
    "XI_CNT_SEL_TRANSPORT_QUEUE_UNDERRUN": c_uint(
        6
    ),  # Incremented when camera starts to transfer new image, however no target buffer is queued in the transport queue. Connected to GenTL.STREAM_INFO_NUM_UNDERRUN. (see Note1)
    "XI_CNT_SEL_ACQUISITION_AUTO_RESTARTED_ON_FAILURE": c_uint(
        7
    ),  # Acquisition can be restarted, due to failures on bus
}

# structure containing information about timestamp reset arming
XI_TS_RST_MODE = {
    "XI_TS_RST_ARM_ONCE": c_uint(
        0
    ),  # Engine is disabled after TimeStamp has been reset after selected event.
    "XI_TS_RST_ARM_PERSIST": c_uint(
        1
    ),  # Engine is armed permanently so each selected event will trigger TimeStamp reset.
}
...
XI_TS_RST_SOURCE = {
    ...
    "XI_TS_RST_SRC_GPI_1": c_uint(
        1
    ),  # GPI1 rising edge is active (signal after de-bounce module)
    ...
    "XI_TS_RST_SRC_GPO_1_INV": c_uint(
        13
    ),  # TimeStamp reset source selected GPO1 inverted
    "XI_TS_RST_SRC_GPO_2_INV": c_uint(
        14
    ),  # TimeStamp reset source selected GPO2 inverted
    "XI_TS_RST_SRC_GPO_3_INV": c_uint(
        15
    ),  # TimeStamp reset source selected GPO3 inverted
    "XI_TS_RST_SRC_GPO_4_INV": c_uint(
        16
    ),  # TimeStamp reset source selected GPO4 inverted
    ...
    "XI_TS_RST_SRC_SW": c_uint(
        19
    ),  # TRIGGER to sensor rising edge is active. TimeStamp is reset by software take effect imminently.
    ...
    "XI_TS_RST_SRC_FVAL": c_uint(
        22
    ),  # Frame valid signal rising edge (internal signal in camera)
    "XI_TS_RST_SRC_FVAL_INV": c_uint(
        23
    ),  # Frame valid signal falling edge (internal signal in camera)
    ...
    "XI_TS_RST_SRC_GPI_7": c_uint(
        28
    ),  # TimeStamp reset source selected GPI7 (after de bounce)
    "XI_TS_RST_SRC_GPI_8": c_uint(
        29
    ),  # TimeStamp reset source selected GPI8 (after de bounce)
    "XI_TS_RST_SRC_GPI_9": c_uint(
        30
    ),  # TimeStamp reset source selected GPI9 (after de bounce)
    "XI_TS_RST_SRC_GPI_10": c_uint(
        31
    ),  # TimeStamp reset source selected GPI10 (after de bounce)
    "XI_TS_RST_SRC_GPI_11": c_uint(
        32
    ),  # TimeStamp reset source selected GPI11 (after de bounce)
    "XI_TS_RST_SRC_GPI_7_INV": c_uint(
        33
    ),  # TimeStamp reset source selected GPI7 inverted (after de bounce)
    "XI_TS_RST_SRC_GPI_8_INV": c_uint(
        34
    ),  # TimeStamp reset source selected GPI8 inverted (after de bounce)
    "XI_TS_RST_SRC_GPI_9_INV": c_uint(
        35
    ),  # TimeStamp reset source selected GPI9 inverted (after de bounce)
    "XI_TS_RST_SRC_GPI_10_INV": c_uint(
        36
    ),  # TimeStamp reset source selected GPI10 inverted (after de bounce)
    "XI_TS_RST_SRC_GPI_11_INV": c_uint(
        37
    ),  # TimeStamp reset source selected GPI11 inverted (after de bounce)
}
...
XI_TEMP_CTRL_MODE_SELECTOR = {
    "XI_TEMP_CTRL_MODE_OFF": c_uint(
        0
    ),  # Controlling of elements (TEC/Peltier, Fans) is turned off
    "XI_TEMP_CTRL_MODE_AUTO": c_uint(
        1
    ),  # Controlling of elements is performed automatically by API or camera in order to reach parameter TARGET_TEMP.
    "XI_TEMP_CTRL_MODE_MANUAL": c_uint(
        2
    ),  # Controlling of elements is done manually by application.
}

# Temperature element selector
XI_TEMP_ELEMENT_SELECTOR = {
    "XI_TEMP_ELEM_TEC1": c_uint(
        11
    ),  # TEC1 = TEC/Peltier that is closest to the image sensor
    "XI_TEMP_ELEM_TEC2": c_uint(
        12
    ),  # TEC2 = TEC/Peltier location depends on camera model
    "XI_TEMP_ELEM_FAN1": c_uint(
        31
    ),  # Temperature element fan current or rotation (FAN1 = Fan)
    "XI_TEMP_ELEM_FAN1_THRS_TEMP": c_uint(
        32
    ),  # Temperature element fan start rotation threshold temperature
}
...
XI_IMAGE_CORRECTION_SELECTOR = {
    "XI_CORRECTION_TYPE_SELECTOR": c_uint(
        0
    ),  # Correction Type selected see XI_TYPE_CORRECTION_SELECTOR
    "XI_DEFECT_ID": c_uint(1),  # Select defect id
    "XI_DEFECTS_COUNT_BY_TYPE": c_uint(
        2
    ),  # Count of defects selected by current XI_DEFECT_TYPE
    ...
    "XI_LIST_STATUS": c_uint(
        17
    ),  # Current list status (Read-only). Result is mask of bits XI_LIST_STATUS_GENERATED, XI_LIST_STATUS_...
    "XI_IMG_COR_TAP_SELECTOR": c_uint(64),  # Selected tap id (0-N) for image correction
    "XI_IMG_COR_GAIN_TUNE": c_uint(
        65
    ),  # Adjustment of gain in dB. For multitap sensors, active tap is selected by XI_IMG_COR_TAP_SELECTOR.
    "XI_IMG_COR_OFFSET_TUNE": c_uint(
        66
    ),  # Adjustment of pixel values offset. For multitap sensors, active tap is selected by XI_IMG_COR_TAP_SELECTOR.
}

# Define image  correction type
XI_TYPE_CORRECTION_SELECTOR = {
    "XI_CORR_TYPE_SENSOR_DEFECTS_FACTORY": c_uint(0),  # Factory defect list
    "XI_CORR_TYPE_SENSOR_COLUMN_FPN": c_uint(
        1
    ),  # Select Fixed Pattern Noise Correction for Columns
    "XI_CORR_TYPE_SENSOR_ADC_BLO": c_uint(
        2
    ),  # ADC gain and black level offset sensor register correction
    "XI_CORR_TYPE_SENSOR_ROW_FPN": c_uint(
        3
    ),  # Select Fixed Pattern Noise Correction for Rows
    "XI_CORR_TYPE_SENSOR_DEFECTS_USER0": c_uint(4),  # User defect list
    "XI_CORR_TYPE_SENSOR_CHANNELS_TUNE": c_uint(
        5
    ),  # Image channel/tap intensity correction
    "XI_CORR_TYPE_SENSOR_COLUMN_BLACK_OFFSET": c_uint(
        6
    ),  # Select image black offset Correction for Columns
    "XI_CORR_TYPE_SENSOR_ROW_BLACK_OFFSET": c_uint(
        7
    ),  # Select image black offset Correction for Rows
    "XI_CORR_TYPE_SENSOR_DEFECTS_IN_CAMERA": c_uint(
        8
    ),  # Custom defect list(specific cameras)
}
...
XI_IMAGE_DEFECT_SUB_TYPE = {
    ...
    "XI_IMAGE_DEFECT_SUB_TYPE_BRIGHT": c_uint(
        1
    ),  # Defect pixel(s) is(are) out of range
...
XI_EXPOSURE_TIME_SELECTOR_TYPE = {
    ...
    "XI_EXPOSURE_TIME_SELECTOR_GROUP1": c_uint(
        1
    ),  # Selects the common Exposure Time for pixel group 1 (for InterlineExposureMode)
    "XI_EXPOSURE_TIME_SELECTOR_GROUP2": c_uint(
        2
    ),  # Selects the common Exposure Time for pixel group 2 (for InterlineExposureMode)
    "XI_EXPOSURE_TIME_SELECTOR_DUAL_TRG_EXP_ZONE_1": c_uint(
        3
    ),  # Selects the Exposure Time for Zone 1 (for Dual Trigger Exposure feature)
    "XI_EXPOSURE_TIME_SELECTOR_DUAL_TRG_EXP_ZONE_2": c_uint(
        4
    ),  # Selects the Exposure Time for Zone 2 (for Dual Trigger Exposure feature)
}
...
XI_GAIN_SELECTOR_TYPE = {
    "XI_GAIN_SELECTOR_ALL": c_uint(
        0
    ),  # Gain selector selects all channels. Implementation of gain type depends on camera.
    "XI_GAIN_SELECTOR_ANALOG_ALL": c_uint(
        1
    ),  # Gain selector selects all analog channels. This is available only on some cameras.
    "XI_GAIN_SELECTOR_DIGITAL_ALL": c_uint(
        2
    ),  # Gain selector selects all digital channels. This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_TAP1": c_uint(
        3
    ),  # Gain selector selects tap 1. This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_TAP2": c_uint(
        4
    ),  # Gain selector selects tap 2. This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_TAP3": c_uint(
        5
    ),  # Gain selector selects tap 3. This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_TAP4": c_uint(
        6
    ),  # Gain selector selects tap 4. This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_N": c_uint(
        7
    ),  # First of two channels of programmable gain control (PGC) function - Gain setting of R, B pixels (North column analog gain). This is available only on some cameras.
    "XI_GAIN_SELECTOR_ANALOG_S": c_uint(
        8
    ),  # Second of two channels of programmable gain control (PGC) function - Gain setting of Gr, Gb pixels (South column analog gain). This is available only on some cameras.
}
...
XI_SHUTTER_TYPE = {
    ...
    "XI_SHUTTER_GLOBAL_RESET_RELEASE": c_uint(
        2
...
XI_LENS_FEATURE = {
    "XI_LENS_FEATURE_MOTORIZED_FOCUS_SWITCH": c_uint(
        1
    ),  # Status of lens motorized focus switch
    "XI_LENS_FEATURE_MOTORIZED_FOCUS_BOUNDED": c_uint(
        2
    ),  # On read = 1 if motorized focus is on one of limits.
    "XI_LENS_FEATURE_MOTORIZED_FOCUS_CALIBRATION": c_uint(
        3
    ),  # (planned feature) On read = 1 if motorized focus is calibrated. Write 1 to start calibration.
    "XI_LENS_FEATURE_IMAGE_STABILIZATION_ENABLED": c_uint(
        4
    ),  # On read = 1 if image stabilization is enabled. Write 1 to enable image stabilization.
    "XI_LENS_FEATURE_IMAGE_STABILIZATION_SWITCH_STATUS": c_uint(
        5
    ),  # On read = 1 if image stabilization switch is in position On.
    "XI_LENS_FEATURE_IMAGE_ZOOM_SUPPORTED": c_uint(
        6
    ),  # On read = 1 if lens supports zoom = are not prime.
}

# Sensor feature selector selects which feature will be accessed.
XI_SENSOR_FEATURE_SELECTOR = {
    "XI_SENSOR_FEATURE_ZEROROT_ENABLE": c_uint(
        0
    ),  # Sensor Zero ROT enable for ONSEMI PYTHON family. For camera model:MQ013xG-ON (on/off)
    "XI_SENSOR_FEATURE_BLACK_LEVEL_CLAMP": c_uint(
        1
    ),  # Black level offset clamping (value). for Camera model:MD
    "XI_SENSOR_FEATURE_MD_FPGA_DIGITAL_GAIN_DISABLE": c_uint(
        2
    ),  # Disable digital component of gain for MD family (1=disabled/0=enabled)
    "XI_SENSOR_FEATURE_ACQUISITION_RUNNING": c_uint(
        3
    ),  # Sensor acquisition is running status (0/1). Could be stopped by setting of 0. For camera model:CB,MC,MX,MT
    "XI_SENSOR_FEATURE_TIMING_MODE": c_uint(
        4
    ),  # Sensor timing mode (value depends on sensor)
    "XI_SENSOR_FEATURE_PARALLEL_ADC": c_uint(
        5
    ),  # Enables the parallel ADC readout mode, where all exposed pixels undergo dual sampling, leading to reduced readout noise at the cost of increased readout time
    "XI_SENSOR_FEATURE_BLACK_LEVEL_OFFSET_RAW": c_uint(
        6
    ),  # Sensor specific register raw black level offset (value)
    "XI_SENSOR_FEATURE_SHORT_INTERVAL_SHUTTER": c_uint(
        7
    ),  # Sensor short Interval Shutter (on/off)
    "XI_SENSOR_FEATURE_AUTO_LOW_POWER_MODE_AUTO": c_uint(
        8
    ),  # Sensor low power mode (on/off)
    "XI_SENSOR_FEATURE_HIGH_CONVERSION_GAIN": c_uint(
        9
    ...
    "XI_SENSOR_FEATURE_DUAL_TRG_EXP_ZONE_DIVIDER_POSITION": c_uint(
        10
    ),  # Sensor Dual Trigger Exposure Zone Divider Position
    "XI_SENSOR_FEATURE_TOF_VCSEL_CTRL_VOLTAGE_MV": c_uint(
        11
    ),  # ToF VCSEL Control Voltage in mV
    "XI_SENSOR_FEATURE_MULTIPLE_ADC": c_uint(
        12
    ),  # Multiple ADC feature performs multiple ADC conversions of the same analog pixel signal and averages the results. It reduces random noise and improves the overall signal-to-noise ratio. For camera model: MX2457 (1=Off, 2=2ADC, 4=4ADC)
    "XI_SENSOR_FEATURE_FAST_TRIGGER_MODE_ENABLE": c_uint(
        13
    ),  # Enables a sensor trigger mode that starts exposure immediately at the beginning of the trigger signal. When fast trigger mode is disabled, the camera operates in a trigger mode that allows exposure and readout to overlap, resulting in a higher FPS. For camera model: MU051xG. (on/off)
}

# Extended feature selector.
XI_EXT_FEATURE_SELECTOR = {
    "XI_EXT_FEATURE_SEL_SIMULATOR_GENERATOR_FRAME_LOST_PERIOD_MIN": c_uint(
        1
    ),  # Camera simulator lost frame generation minimum period (in frames).
    "XI_EXT_FEATURE_SEL_SIMULATOR_GENERATOR_FRAME_LOST_PERIOD_MAX": c_uint(
        2
    ),  # Camera simulator lost frame generation random period (in frames).
    "XI_EXT_FEATURE_SEL_SIMULATOR_IMAGE_DATA_FORMAT": c_uint(
        3
    ),  # Camera simulator image data format.
    "XI_EXT_FEATURE_SEL_BANDWIDTH_MEASUREMENT_TIME_SECONDS": c_uint(
        4
    ),  # Number of seconds for bandwidth measurement. Default = 1.
    "XI_EXT_FEATURE_SEL_IMAGE_INTENSIFIER_VOLTAGE": c_uint(
        5
    ),  # Input voltage for image intensifier. Default = 0.
    "XI_EXT_FEATURE_SEL_TRIG_FRAME": c_uint(
        6
    ),  # Triggers frame(s) on internal event. Default = 0.
    "XI_EXT_FEATURE_SEL_IMAGE_OVERSAMPLING": c_uint(
        7
    ),  # Enable/disable image pixels oversampling. Default = 0.
    "XI_EXT_FEATURE_SEL_APPLY_DATA_FINAL": c_uint(
        8
    ),  # Enable/disable applying data final. Default = 1.
    "XI_EXT_FEATURE_SEL_FAN_RPM": c_uint(
        9
    ),  # Sets camera cooling fan rpm (% from max). Default = 100.
    "XI_EXT_FEATURE_SEL_DITHERING_HOST": c_uint(
        10
    ),  # Enables/Disables shifted(left/up) image data dithering on HOST side. Default = 0(off).
    "XI_EXT_FEATURE_SEL_DITHERING_DEVICE": c_uint(
        11
    ),  # Enables/Disables shifted(left/up) image data dithering on DEVICE side. Default = 0(off).
    "XI_EXT_FEATURE_SEL_FAN_THR_TEMP": c_uint(
        12
    ),  # Sets camera fan/back side threshold temperature. Default = 35.
    "XI_EXT_FEATURE_PCIE_IOCTL_GLOBAL_LOCK_ENABLED": c_uint(
        13
    ),  # Controls if PCIe IOCTL global locking is enabled. If disabled, concurrent operation (e.g. using filesystem is running faster in multiple threads)
    "XI_EXT_FEATURE_SEL_EXTERNAL_POWER_SOURCE_VOLTAGE": c_uint(
        14
    ),  # Input voltage from external power source.
    "XI_EXT_FEATURE_SEL_TOL_INV_SBSN": c_uint(
        15
    ),  # Tolerance to invalid Sensor board SN while open on certain cameras.
    "XI_EXT_FEATURE_SEL_TOL_FRAME_BUFFER_RAM_INIT_ERROR": c_uint(
        16
    ),  # Tolerance FrameBuffer RAM initialization error while open.
    "XI_EXT_FEATURE_SEL_TRANSPARENT_RETURN_CODES_ENABLED": c_uint(
        17
    ),  # Enables transparent return codes(MC family).
    "XI_EXT_FEATURE_SEL_TARGET_TEMPERATURE_MINIMUM": c_uint(
        18
    ),  # Allows to set minimum limit of target temperature.
    "XI_EXT_FEATURE_SEL_PRINT_STATUS_TO_DEBUG": c_uint(
        19
    ),  # Prints to debug status information on some scientific cameras e.g. MJ (tepmerature, consumption and other parameters).
    "XI_EXT_FEATURE_SEL_BUFFER_HEADER_CHECK_ENABLED": c_uint(
        20
    ),  # Input buffer integrity check.
    "XI_EXT_FEATURE_SEL_TRANSPORT_IMAGE_BUFFER_SIZE_ALIGNMENT_BYTES": c_uint(
        21
    ),  # Size automatic aligment for transport image buffer size to be automatically aligned. E.g. 65536 for 64KiB, useful if storing (e.g. to SSD) requires larger sizes, than image payload. Default is 4096 bytes.
    "XI_EXT_FEATURE_SEL_PROCESSING_IBD_CROP_MSB_EN": c_uint(
        22
    ),  # Crop MSB bits of resizer IP data output (if enabled output data format bitdepth of resize IP equal to input format)
    "XI_EXT_FEATURE_API_SETPARAM_CALLS_LOG_ENABLED": c_uint(
        23
    ),  # Allows enabling of xiSetParam calls to be logged automatically to file xiapi_call_log.txt
}
...
XI_DEVICE_UNIT_REGISTER_TYPE = {
    ...
    "XI_REGISTER_TYPE_INT100X": c_uint(
        2
    ),  # Integer value register type which is a 100 fold of the target value
    "XI_REGISTER_TYPE_INT1000X": c_uint(
        3
    ),  # Integer value register type which is a 1000 fold of the target value
    "XI_REGISTER_TYPE_INT32768X": c_uint(
        4
    ),  # Integer value register type which is a 32768 fold of the target value
    "XI_REGISTER_TYPE_INT_HEX": c_uint(
        5
    ),  # Integer value register type represented in hex format
    "XI_REGISTER_TYPE_ENUM_INT1000x": c_uint(
        6
    ),  # Enumerated value register type representing 1000 fold of gain values
    "XI_REGISTER_TYPE_ENUM_RAW": c_uint(
        7
    ),  # Enumerated value register type representing raw gain value stored in EEPROM
}

# Device unit register
XI_DEVICE_UNIT_MCU1_REGS = {
    "XI_DEVICE_UNIT_MCU1_ACQ_RESTARTED": c_uint(
        0
    ),  # How many times acquisition was restarted, due to errors.
}

# Device unit register
XI_DEVICE_UNIT_I2C_BUS_REGS = {
    "XI_DEVICE_UNIT_I2C_BUS_SLAVE_ADDR": c_uint(0),  # i2c bus slave address.
    "XI_DEVICE_UNIT_I2C_BUS_SLAVE_SUB_ADDR_LEN": c_uint(
        1
    ),  # i2c bus slave sub address lenght in bytes.
    "XI_DEVICE_UNIT_I2C_BUS_SLAVE_SUB_ADDR": c_uint(
        2
    ),  # i2c bus slave sub address (LSB first).
    "XI_DEVICE_UNIT_I2C_BUS_SLAVE_DATA_LEN": c_uint(
        3
    ),  # i2c bus data read/write lenght.
    "XI_DEVICE_UNIT_I2C_BUS_SLAVE_DATA": c_uint(
        4
    ),  # i2c bus data read/write. Transaction will be executed on last byte written or first byte read.
}

# Device unit register type
XI_DEVICE_UNIT_XIFAPI_ACQ_REGS = {
    "XI_DEVICE_UNIT_XIFAPI_ACQ_LOOP_TIME_ENABLE": c_uint(
        0
    ),  # Enables calculation of worker loops time
    "XI_DEVICE_UNIT_XIFAPI_ACQ_RESET": c_uint(1),  # Resets all calculated,cached data
    "XI_DEVICE_UNIT_XIFAPI_ACQ_LOOP_TIME": c_uint(
        2
    ),  # Selects register of one worker loop time
    "XI_DEVICE_UNIT_XIFAPI_ACQ_LOOPS_TIME": c_uint(
        3
    ),  # Selects register of worker loops time(since last reset)
    "XI_DEVICE_UNIT_XIFAPI_ACQ_LOOPS": c_uint(
        4
    ),  # Selects register of worker number of loops(since last reset)
    "XI_DEVICE_UNIT_XIFAPI_WORKER_INTERRUPT_SIM_TIME": c_uint(
        5
    ),  # Simulates waorker interruption (in ms)
}
...
XI_IMAGE_AREA_SELECTOR = {
    "XI_IMAGE_AREA_ACTIVE": c_uint(
        0
    ),  # All light sensitive pixels suggested by image vendor.
    "XI_IMAGE_AREA_ACTIVE_AND_MASKED": c_uint(
        1
    ),  # All Active pixels plus masked pixels surrounding the Active area.
}
...
XI_SENS_DEFFECTS_CORR_LIST_SELECTOR = {
    "XI_SENS_DEFFECTS_CORR_LIST_SEL_FACTORY": c_uint(
        0
    ),  # Factory defect correction list
    "XI_SENS_DEFFECTS_CORR_LIST_SEL_USER0": c_uint(1),  # User defect correction list
    "XI_SENS_DEFFECTS_CORR_LIST_SEL_IN_CAMERA": c_uint(
        2
    ),  # Device specific defect correction list
}

# Acquisition status Selector
XI_ACQUISITION_STATUS_SELECTOR = {
    "XI_ACQUISITION_STATUS_ACQ_ACTIVE": c_uint(
        0
...
XI_REGIONS_GEOMETRY = {
    ...
    "XI_REGIONS_GEOMETRY_MULTIPLE_REGIONS_IN_SINGLE_COLUMN": c_uint(
        1
    ),  # Multiple regions in single column
    "XI_REGIONS_GEOMETRY_MULTIPLE_REGIONS_IN_MULTIPLE_COLUMNS": c_uint(
        2
    ),  # Multiple regions in multiple columns
}
...
XI_DP_PROC_SELECTOR = {
    ...
    "XI_DP_PROC_CHANNEL_MUXER": c_uint(
        1
    ),  # Channel Muxer (selected processor combines multiple input channels)
...
XI_DP_PARAM_SELECTOR = {
    ...
    "XI_DP_PARAM_CHMUX_CHANNEL_SELECTOR": c_uint(
        1
    ),  # Defines output of Channel Muxer processor
...
XI_DP_PARAM_VALUE = {
    ...
    "XI_DP_PARAM_VALUE_PIXSEQ_FOUR_VALUES": c_uint(
        11
    ),  # Output are four values per pixel
    "XI_DP_PARAM_VALUE_CHMUX_CMS_A": c_uint(
        12
    ),  # Correlated Multiple Sampling(averaging)
}
...
XI_USER_SET_SELECTOR = {
    ...
    "XI_US_2_12_CMS_S_L": c_uint(
        14
    ),  # 12bit per channel, 2 samples,  CMS(summing) Low Gain mode preset.
    "XI_US_2_12_CMS_S_H": c_uint(
        15
    ),  # 12bit per channel, 2 samples,  CMS(summing) High Gain mode preset.
    "XI_US_2_14_CMS_S_L": c_uint(
        16
    ),  # 14bit per channel, 2 samples,  CMS(summing) Low Gain mode preset.
    "XI_US_2_14_CMS_S_H": c_uint(
        17
    ),  # 14bit per channel, 2 samples,  CMS(summing) High Gain mode preset.
    "XI_US_4_12_CMS_S_L": c_uint(
        18
    ),  # 12bit per channel, 4 samples,  CMS(summing) Low Gain mode preset.
    "XI_US_4_12_CMS_S_H": c_uint(
        19
    ),  # 12bit per channel, 4 samples,  CMS(summing) High Gain mode preset.
    "XI_US_4_14_CMS_S_L": c_uint(
        20
    ),  # 14bit per channel, 4 samples,  CMS(summing) Low Gain mode preset.
    "XI_US_4_14_CMS_S_H": c_uint(
        21
    ),  # 14bit per channel, 4 samples,  CMS(summing) High Gain mode preset.
    "XI_US_2_12_HDR_HL": c_uint(
        22
    ),  # 12bit per channel, 2 samples,  HDR High Low Gain mode preset.
    "XI_US_2_12_HDR_L": c_uint(
        23
    ),  # 12bit per channel, 2 samples,  HDR Low Gain mode preset.
    "XI_US_2_12_HDR_H": c_uint(
        24
    ),  # 12bit per channel, 2 samples,  HDR High Gain mode preset.
    "XI_US_4_12_CMS_HDR_HL": c_uint(
        25
    ),  # 12bit per channel, 4 samples,  CMS + HDR High Low Gain mode preset.
    "XI_US_2_14_HDR_L": c_uint(
        26
    ),  # 14bit per channel, 2 samples,  HDR Low Gain mode preset.
    "XI_US_2_14_HDR_H": c_uint(
        27
    ),  # 14bit per channel, 2 samples,  HDR High Gain mode preset.
    "XI_US_2_12_CMS_A_L": c_uint(
        28
    ),  # 12bit per channel, 2 samples,  CMS(averaging) Low Gain mode preset.
    "XI_US_2_12_CMS_A_H": c_uint(
        29
    ),  # 12bit per channel, 2 samples,  CMS(averaging) High Gain mode preset.
...
# Probe Selector
XI_PROBE_SELECTOR = {
    "XI_PROBE_SELECTOR_CURRENT_MAINBOARD_VCC_IN": c_uint(
        0
    ),  # Current probe on Main Board at VCC_IN power supply
    "XI_PROBE_SELECTOR_VOLTAGE_MAINBOARD_VCC_IN": c_uint(
        1
    ),  # Voltage probe on Main Board at VCC_IN power supply
    "XI_PROBE_SELECTOR_CURRENT_MAINBOARD_VCC_ADJ2": c_uint(
        2
    ),  # Current probe on Main Board at VCC_ADJ2 power supply
    "XI_PROBE_SELECTOR_VOLTAGE_MAINBOARD_VCC_ADJ2": c_uint(
        3
    ),  # Voltage probe on Main Board at VCC_ADJ2 power supply
    "XI_PROBE_SELECTOR_CURRENT_MAINBOARD_VCC_ADJ1": c_uint(
        4
    ),  # Current probe on Main Board at VCC_ADJ1 power supply
    "XI_PROBE_SELECTOR_VOLTAGE_MAINBOARD_VCC_ADJ1": c_uint(
        5
    ),  # Voltage probe on Main Board at VCC_ADJ1 power supply
    "XI_PROBE_SELECTOR_CURRENT_MAINBOARD_VCC_PLT": c_uint(
        6
    ),  # Current probe on Main Board at VCC_PLT power supply
    "XI_PROBE_SELECTOR_VOLTAGE_MAINBOARD_VCC_PLT": c_uint(
        7
    ),  # Voltage probe on Main Board at VCC_PLT power supply
    "XI_PROBE_SELECTOR_VOLTAGE_SENSORBOARD_VCC_ADJ1": c_uint(
        8
    ),  # Voltage probe on Sensor Board at VCC_ADJ1
    "XI_PROBE_SELECTOR_VOLTAGE_SENSORBOARD_VCC_ADJ2": c_uint(
        9
    ),  # Voltage probe on Sensor Board at VCC_ADJ2
    "XI_PROBE_SELECTOR_VOLTAGE_SENSORBOARD_VCC_5V0": c_uint(
        10
    ),  # Voltage probe on Sensor Board at VCC_5V0
    "XI_PROBE_SELECTOR_VOLTAGE_SENSORBOARD_VCC_3V3": c_uint(
        11
    ),  # Voltage probe on Sensor Board at VCC_3V3
    "XI_PROBE_SELECTOR_VOLTAGE_DATA_CON_INPUT": c_uint(
        12
    ),  # Voltage probe on device input, if device is bus powered
    ...
    "XI_PROBE_SELECTOR_VOLTAGE_MAINBOARD_VCC_5V0": c_uint(
        17
    ),  # Voltage probe on Main Board at VCC_5V0 power supply
    "XI_PROBE_SELECTOR_CURRENT_MAINBOARD_VCC_5V0": c_uint(
        18
    ),  # Current probe on Main Board at VCC_5V0 power supply
}
...
XI_DATA_SM = {
    "XI_DATA_SM_UNSIGNED": c_uint(
        0
    ),  # Unsigned if it can only represent non-negative numbers (zero or positive numbers).
    "XI_DATA_SM_SIGNED_2C": c_uint(
        1
    ),  # Signed if it can represent both positive and negative numbers (two's complement).
...
XI_GenTL_Image_Format_e = {
    "XI_GenTL_Image_Format_Mono8": c_uint(
        0x01080001
    ),  # This enumeration value sets the pixel format to Mono 8.
    "XI_GenTL_Image_Format_BGRA8": c_uint(
        0x02200017
    ),  # This enumeration value sets the pixel format to Bayer BG 8
    "XI_GenTL_Image_Format_RGB8Planar": c_uint(
        0x02180021
    ...
    "XI_GenTL_Image_Format_RGB8": c_uint(
        0x02180014
    ),  # This enumeration value sets the pixel format to R8,G8,B8
    "XI_GenTL_Image_Format_BGR8": c_uint(
        0x02180015
    ),  # This enumeration value sets the pixel format to B8,G8,R8
    "XI_GenTL_Image_Format_BayerRG14": c_uint(
        0x0110010A
    ),  # Pixel format one 14 bit pixel in 2 bytes
    "XI_GenTL_Image_Format_BayerGR14": c_uint(
        0x01100109
    ),  # Pixel format one 14 bit pixel in 2 bytes
    "XI_GenTL_Image_Format_BayerBG14": c_uint(
        0x0110010C
    ),  # Pixel format one 14 bit pixel in 2 bytes
    "XI_GenTL_Image_Format_BayerGB14": c_uint(
        0x0110010B
    ),  # Pixel format one 14 bit pixel in 2 bytes
    "XI_GenTL_Image_Format_BayerBG10p": c_uint(
        0x010A0052
    ),  # Pixel format 10bit packed (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_BayerGB10p": c_uint(
        0x010A0054
    ),  # Pixel format 10bit packed (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_BayerGR10p": c_uint(
        0x010A0056
    ),  # Pixel format 10bit packed (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_BayerRG10p": c_uint(
        0x010A0058
    ),  # Pixel format 10bit packed (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_Mono10p": c_uint(
        0x010A0046
    ),  # Pixel format 10bit packed (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_BayerBG12p": c_uint(
        0x010C0053
    ),  # Pixel format 12bit packed (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_BayerGB12p": c_uint(
        0x010C0055
    ),  # Pixel format 12bit packed (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_BayerGR12p": c_uint(
        0x010C0057
    ),  # Pixel format 12bit packed (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_BayerRG12p": c_uint(
        0x010C0059
    ),  # Pixel format 12bit packed (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_Mono12p": c_uint(
        0x010C0047
    ),  # Pixel format 12bit packed (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_BayerBG14p": c_uint(
        0x010E0108
    ),  # Pixel format 14bit packed (4 pixels in 7 bytes)
    "XI_GenTL_Image_Format_BayerGB14p": c_uint(
        0x010E0107
    ),  # Pixel format 14bit packed (4 pixels in 7 bytes)
    "XI_GenTL_Image_Format_BayerGR14p": c_uint(
        0x010E0105
    ),  # Pixel format 14bit packed (4 pixels in 7 bytes)
    "XI_GenTL_Image_Format_BayerRG14p": c_uint(
        0x010E0106
    ),  # Pixel format 14bit packed (4 pixels in 7 bytes)
    "XI_GenTL_Image_Format_Mono14p": c_uint(
        0x010E0104
    ),  # Pixel format 14bit packed (4 pixels in 7 bytes)
    "XI_GenTL_Image_Format_xiBayerBG10g160": c_uint(
        0x8200010A
    ),  # Pixel format 10bit (16 pixels in 20 bytes)
    "XI_GenTL_Image_Format_xiBayerGB10g160": c_uint(
        0x8200020A
    ),  # Pixel format 10bit (16 pixels in 20 bytes)
    "XI_GenTL_Image_Format_xiBayerGR10g160": c_uint(
        0x8200030A
    ),  # Pixel format 10bit (16 pixels in 20 bytes)
    "XI_GenTL_Image_Format_xiBayerRG10g160": c_uint(
        0x8200040A
    ),  # Pixel format 10bit (16 pixels in 20 bytes)
    "XI_GenTL_Image_Format_xiMono10g160": c_uint(
        0x8200000A
    ),  # Pixel format 10bit (16 pixels in 20 bytes)
    "XI_GenTL_Image_Format_xiBayerBG12g192": c_uint(
        0x8200010C
    ),  # Pixel format 12bit (16 pixels in 24 bytes)
    "XI_GenTL_Image_Format_xiBayerGB12g192": c_uint(
        0x8200020C
    ),  # Pixel format 12bit (16 pixels in 24 bytes)
    "XI_GenTL_Image_Format_xiBayerGR12g192": c_uint(
        0x8200030C
    ),  # Pixel format 12bit (16 pixels in 24 bytes)
    "XI_GenTL_Image_Format_xiBayerRG12g192": c_uint(
        0x8200040C
    ),  # Pixel format 12bit (16 pixels in 24 bytes)
    "XI_GenTL_Image_Format_xiMono12g192": c_uint(
        0x8200000C
    ),  # Pixel format 12bit (16 pixels in 24 bytes)
    "XI_GenTL_Image_Format_xiBayerBG14g224": c_uint(
        0x8200010E
    ),  # Pixel format 14bit (16 pixels in 28 bytes)
    "XI_GenTL_Image_Format_xiBayerGB14g224": c_uint(
        0x8200020E
    ),  # Pixel format 14bit (16 pixels in 28 bytes)
    "XI_GenTL_Image_Format_xiBayerGR14g224": c_uint(
        0x8200030E
    ),  # Pixel format 14bit (16 pixels in 28 bytes)
    "XI_GenTL_Image_Format_xiBayerRG14g224": c_uint(
        0x8200040E
    ),  # Pixel format 14bit (16 pixels in 28 bytes)
    "XI_GenTL_Image_Format_xiMono14g224": c_uint(
        0x8200000E
    ),  # Pixel format 14bit (16 pixels in 28 bytes)
    "XI_GenTL_Image_Format_xiMono8TS01": c_uint(
        0x80010008
    ),  # Pixel format 8bit(2 taps)
    "XI_GenTL_Image_Format_xiMono10TS01": c_uint(
        0x8001000A
    ),  # Pixel format 10bit(2 taps)
    "XI_GenTL_Image_Format_xiMono12TS01": c_uint(
        0x8001000C
    ),  # Pixel format 12bit(2 taps)
    "XI_GenTL_Image_Format_xiMono14TS01": c_uint(
        0x8001000E
    ),  # Pixel format 14bit(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG8TS01": c_uint(0x80010408),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerRG10TS01": c_uint(
        0x8001040A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG12TS01": c_uint(
        0x8001040C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG14TS01": c_uint(
        0x8001040E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG8TS01": c_uint(0x80010108),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerBG10TS01": c_uint(
        0x8001010A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG12TS01": c_uint(
        0x8001010C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG14TS01": c_uint(
        0x8001010E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB8TS01": c_uint(0x80010208),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerGB10TS01": c_uint(
        0x8001020A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB12TS01": c_uint(
        0x8001020C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB14TS01": c_uint(
        0x8001020E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR8TS01": c_uint(0x80010308),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerGR10TS01": c_uint(
        0x8001030A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR12TS01": c_uint(
        0x8001030C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR14TS01": c_uint(
        0x8001030E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiMono8TS03": c_uint(
        0x80030008
    ),  # Pixel format 8bit(4 taps)
    "XI_GenTL_Image_Format_xiMono10TS03": c_uint(
        0x8003000A
    ),  # Pixel format 10bit(4 taps)
    "XI_GenTL_Image_Format_xiMono12TS03": c_uint(
        0x8003000C
    ),  # Pixel format 12bit(4 taps)
    "XI_GenTL_Image_Format_xiMono14TS03": c_uint(
        0x8003000E
    ),  # Pixel format 14bit(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG8TS03": c_uint(0x80030408),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerRG10TS03": c_uint(
        0x8003040A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG12TS03": c_uint(
        0x8003040C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG14TS03": c_uint(
        0x8003040E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG8TS03": c_uint(0x80030108),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerBG10TS03": c_uint(
        0x8003010A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG12TS03": c_uint(
        0x8003010C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG14TS03": c_uint(
        0x8003010E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB8TS03": c_uint(0x80030208),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerGB10TS03": c_uint(
        0x8003020A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB12TS03": c_uint(
        0x8003020C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB14TS03": c_uint(
        0x8003020E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR8TS03": c_uint(0x80030308),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerGR10TS03": c_uint(
        0x8003030A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR12TS03": c_uint(
        0x8003030C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR14TS03": c_uint(
        0x8003030E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    ...
    "XI_GenTL_Image_Format_xiMono16TS03": c_uint(
        0x80030010
    ),  # Pixel format 16bit(4 taps)
    "XI_GenTL_Image_Format_xiMono16TS01": c_uint(
        0x80010010
    ),  # Pixel format 16bit(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG16TS01": c_uint(
        0x80010410
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG16TS01": c_uint(
        0x80010110
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB16TS01": c_uint(
        0x80010210
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR16TS01": c_uint(
        0x80010310
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG16TS03": c_uint(
        0x80030410
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG16TS03": c_uint(
        0x80030110
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB16TS03": c_uint(
        0x80030210
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR16TS03": c_uint(
        0x80030310
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiMono16TS04": c_uint(
        0x80040010
    ),  # Pixel format 16bit(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG16TS04": c_uint(
        0x80040410
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG16TS04": c_uint(
        0x80040110
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB16TS04": c_uint(
        0x80040210
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR16TS04": c_uint(
        0x80040310
    ),  # Pixel format 16 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiMono16TS02": c_uint(
        0x80020010
    ),  # Pixel format 16bit(2 taps)
    "XI_GenTL_Image_Format_xiMono8TS02": c_uint(
        0x80020008
    ),  # Pixel format 8bit(2 taps)
    "XI_GenTL_Image_Format_xiMono10TS02": c_uint(
        0x8002000A
    ),  # Pixel format 10bit(2 taps)
    "XI_GenTL_Image_Format_xiMono12TS02": c_uint(
        0x8002000C
    ),  # Pixel format 12bit(2 taps)
    "XI_GenTL_Image_Format_xiMono14TS02": c_uint(
        0x8002000E
    ),  # Pixel format 14bit(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG8TS02": c_uint(0x80020408),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerRG10TS02": c_uint(
        0x8002040A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG12TS02": c_uint(
        0x8002040C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG14TS02": c_uint(
        0x8002040E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerRG16TS02": c_uint(
        0x80020410
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG8TS02": c_uint(0x80020108),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerBG10TS02": c_uint(
        0x8002010A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG12TS02": c_uint(
        0x8002010C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG14TS02": c_uint(
        0x8002010E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerBG16TS02": c_uint(
        0x80020110
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB8TS02": c_uint(0x80020208),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerGB10TS02": c_uint(
        0x8002020A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB12TS02": c_uint(
        0x8002020C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB14TS02": c_uint(
        0x8002020E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGB16TS02": c_uint(
        0x80020210
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR8TS02": c_uint(0x80020308),  # Pixel format (2 taps)
    "XI_GenTL_Image_Format_xiBayerGR10TS02": c_uint(
        0x8002030A
    ),  # Pixel format 10 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR12TS02": c_uint(
        0x8002030C
    ),  # Pixel format 12 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR14TS02": c_uint(
        0x8002030E
    ),  # Pixel format 14 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiBayerGR16TS02": c_uint(
        0x80020310
    ),  # Pixel format 16 bits in 2 bytes(2 taps)
    "XI_GenTL_Image_Format_xiMono8TS04": c_uint(
        0x80040008
    ),  # Pixel format 8bit(4 taps)
    "XI_GenTL_Image_Format_xiMono10TS04": c_uint(
        0x8004000A
    ),  # Pixel format 10bit(4 taps)
    "XI_GenTL_Image_Format_xiMono12TS04": c_uint(
        0x8004000C
    ),  # Pixel format 12bit(4 taps)
    "XI_GenTL_Image_Format_xiMono14TS04": c_uint(
        0x8004000E
    ),  # Pixel format 14bit(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG8TS04": c_uint(0x80040408),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerRG10TS04": c_uint(
        0x8004040A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG12TS04": c_uint(
        0x8004040C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerRG14TS04": c_uint(
        0x8004040E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG8TS04": c_uint(0x80040108),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerBG10TS04": c_uint(
        0x8004010A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG12TS04": c_uint(
        0x8004010C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerBG14TS04": c_uint(
        0x8004010E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB8TS04": c_uint(0x80040208),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerGB10TS04": c_uint(
        0x8004020A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB12TS04": c_uint(
        0x8004020C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGB14TS04": c_uint(
        0x8004020E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR8TS04": c_uint(0x80040308),  # Pixel format (4 taps)
    "XI_GenTL_Image_Format_xiBayerGR10TS04": c_uint(
        0x8004030A
    ),  # Pixel format 10 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR12TS04": c_uint(
        0x8004030C
    ),  # Pixel format 12 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_xiBayerGR14TS04": c_uint(
        0x8004030E
    ),  # Pixel format 14 bits in 2 bytes(4 taps)
    "XI_GenTL_Image_Format_Mono9p": c_uint(
        0x81000009
    ),  # Pixel format 9bit packed (8 pixels in 9 bytes)
    "XI_GenTL_Image_Format_BayerBG9p": c_uint(
        0x81000109
    ),  # Pixel format 9bit packed (8 pixels in 9 bytes)
    "XI_GenTL_Image_Format_BayerGB9p": c_uint(
        0x81000209
    ),  # Pixel format 9bit packed (8 pixels in 9 bytes)
    "XI_GenTL_Image_Format_BayerGR9p": c_uint(
        0x81000309
    ),  # Pixel format 9bit packed (8 pixels in 9 bytes)
    "XI_GenTL_Image_Format_BayerRG9p": c_uint(
        0x81000409
    ),  # Pixel format 9bit packed (8 pixels in 9 bytes)
    "XI_GenTL_Image_Format_Mono11p": c_uint(
        0x8100000B
    ),  # Pixel format 11bit packed (8 pixels in 11 bytes)
    "XI_GenTL_Image_Format_BayerBG11p": c_uint(
        0x8100010B
    ),  # Pixel format 11bit packed (8 pixels in 11 bytes)
    "XI_GenTL_Image_Format_BayerGB11p": c_uint(
        0x8100020B
    ),  # Pixel format 11bit packed (8 pixels in 11 bytes)
    "XI_GenTL_Image_Format_BayerGR11p": c_uint(
        0x8100030B
    ),  # Pixel format 11bit packed (8 pixels in 11 bytes)
    "XI_GenTL_Image_Format_BayerRG11p": c_uint(
        0x8100040B
    ),  # Pixel format 11bit packed (8 pixels in 11 bytes)
    ...
    "XI_GenTL_Image_Format_RGBA8": c_uint(
        0x02200016
    ),  # This enumeration value sets the pixel format to Bayer RG 8
    "XI_GenTL_Image_Format_RGB16Planar": c_uint(
        0x02300024
    ),  # This enumeration value sets the pixel format to RGB 16 Planar.
    "XI_GenTL_Image_Format_BGRA16": c_uint(
        0x02400051
    ),  # This enumeration value sets the pixel format to Bayer BG 16
    "XI_GenTL_Image_Format_BGR16": c_uint(
        0x0230004B
    ),  # This enumeration value sets the pixel format to Bayer BG 16
    "XI_GenTL_Image_Format_RGBA16": c_uint(
        0x02400064
    ),  # This enumeration value sets the pixel format to Bayer RG 16
    "XI_GenTL_Image_Format_RGB16": c_uint(
        0x02300033
    ),  # This enumeration value sets the pixel format to Bayer RG 16
    ...
    "XI_GenTL_Image_Format_xiPaBayerBG8": c_uint(
        0x80000608
    ),  # Bayer BG 8 Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaBayerBG10": c_uint(
        0x8000060A
    ),  # Bayer BG 10 Polarized 4 pixels
    ...
    "XI_GenTL_Image_Format_xiPaBayerBG10p": c_uint(
        0x8100060A
    ),  # Bayer BG 10p Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaBayerBG12": c_uint(
        0x8000060C
    ),  # Bayer BG 12 Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaBayerBG12p": c_uint(
        0x8100060C
    ),  # Bayer BG 12p Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaMono8": c_uint(0x80000508),  # Mono 8 Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaMono10": c_uint(
        0x8000050A
    ),  # Mono 10 Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaMono10p": c_uint(
        0x8100050A
    ),  # Mono 10p Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaMono12": c_uint(
        0x8000050C
    ),  # Mono 12 Polarized 4 pixels
    "XI_GenTL_Image_Format_xiPaMono12p": c_uint(
        0x8100050C
    ),  # Mono 12p Polarized 4 pixels
    ...
    "XI_GenTL_Image_Format_xiMono12g24": c_uint(
        0x8400000C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiMono10g40": c_uint(
        0x8400000A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerRG12g24": c_uint(
        0x8400040C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiBayerGR10g40": c_uint(
        0x8400030A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerGR12g24": c_uint(
        0x8400030C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiBayerGB10g40": c_uint(
        0x8400020A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerGB12g24": c_uint(
        0x8400020C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiBayerBG12g24": c_uint(
        0x8400010C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiBayerRG10g40": c_uint(
        0x8400040A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerBG10g40": c_uint(
        0x8400010A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiToF12g24AnB": c_uint(
        0x84AB0F0C
    ),  # Pixel format 12bit ToF A and B
    "XI_GenTL_Image_Format_xiToF12sg24AmB": c_uint(
        0x84A05F0C
    ),  # Pixel format 12bit signed ToF A - B
    "XI_GenTL_Image_Format_xiToF12g24ApB": c_uint(
        0x84A10F0C
    ),  # Pixel format 12bit ToF A + B
    "XI_GenTL_Image_Format_xiToF12sAmB": c_uint(
        0x80A05F0C
    ),  # Pixel format 12bit signed ToF A - B (A-B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF12g24Aon": c_uint(
        0x84A20F0C
    ),  # Pixel format 12bit ToF A only
    "XI_GenTL_Image_Format_xiToF12g24Bon": c_uint(
        0x84A30F0C
    ),  # Pixel format 12bit ToF B only
    "XI_GenTL_Image_Format_xiToF8sAmB": c_uint(
        0x80A05F08
    ),  # Pixel format 8bit ToF A - B (A-B in 1 byte)  (signed)
    "XI_GenTL_Image_Format_xiToF12AnB": c_uint(
        0x80AB0F0C
    ),  # Pixel format 12bit ToF A and B (A in 2 bytes, B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF12Aon": c_uint(
        0x80A20F0C
    ),  # Pixel format 12bit ToF A only (A in 2 bytes)
    "XI_GenTL_Image_Format_xiToF12ApB": c_uint(
        0x80A10F0C
    ),  # Pixel format 12bit ToF A + B (A+B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF12Bon": c_uint(
        0x80A30F0C
    ),  # Pixel format 12bit ToF B only (B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF8AnB": c_uint(
        0x80AB0F08
    ),  # Pixel format 8bit ToF A and B (A in 1 byte, B in 1 byte)
    "XI_GenTL_Image_Format_xiToF8Aon": c_uint(
        0x80A20F08
    ),  # Pixel format 8bit ToF A only (A in 1 byte)
    "XI_GenTL_Image_Format_xiToF8ApB": c_uint(
        0x80A10F08
    ),  # Pixel format 8bit ToF A + B (A+B in 1 byte)
    "XI_GenTL_Image_Format_xiToF8Bon": c_uint(
        0x80A30F08
    ),  # Pixel format 8bit ToF B only (B in 1 byte)
    "XI_GenTL_Image_Format_xiToF16sAmB": c_uint(
        0x80A05F10
    ),  # Pixel format 16bit signed ToF A - B (A-B in 2 bytes)
    "XI_GenTL_Image_Format_xiMono12g24MS61": c_uint(
        0x8461000C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    "XI_GenTL_Image_Format_xiMono10g40MS61": c_uint(
        0x8461020A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerRG10g40MS61": c_uint(
        0x8461040A
    ),  # Pixel format 10bit (4 pixels in 5 bytes)
    "XI_GenTL_Image_Format_xiBayerRG12g24MS61": c_uint(
        0x8461040C
    ),  # Pixel format 12bit (2 pixels in 3 bytes)
    ...
    "XI_GenTL_Image_Format_xiBayerBG10pLS3A": c_uint(
        0x813A010A
    ),  # Pixel format 10bit packed (PFNC)
    "XI_GenTL_Image_Format_xiMono10LS3A": c_uint(0x803A000A),  # Pixel format 10bit
    "XI_GenTL_Image_Format_xiMono10pLS3A": c_uint(
        0x813A000A
    ),  # Pixel format 10bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerGB10LS3A": c_uint(0x803A020A),  # Pixel format 10bit
    "XI_GenTL_Image_Format_xiBayerGB10pLS3A": c_uint(
        0x813A020A
    ),  # Pixel format 10bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerGR10pLS3A": c_uint(
        0x813A030A
    ),  # Pixel format 10bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerRG10pLS3A": c_uint(
        0x813A040A
    ),  # Pixel format 10bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerBG12pLS3A": c_uint(
        0x813A010C
    ),  # Pixel format 12bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerGB12pLS3A": c_uint(
        0x813A020C
    ),  # Pixel format 12bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerGR12pLS3A": c_uint(
        0x813A030C
    ),  # Pixel format 12bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiBayerRG12pLS3A": c_uint(
        0x813A040C
    ),  # Pixel format 12bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiMono12pLS3A": c_uint(
        0x813A000C
    ),  # Pixel format 12bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiMono14pLS3A": c_uint(
        0x813A000E
    ),  # Pixel format 14bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerBG14pLS3A": c_uint(
        0x813A010E
    ),  # Pixel format 14bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerGB14pLS3A": c_uint(
        0x813A020E
    ),  # Pixel format 14bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerGR14pLS3A": c_uint(
        0x813A030E
    ),  # Pixel format 14bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerRG14pLS3A": c_uint(
        0x813A040E
    ),  # Pixel format 14bit packed (PFNC)
    "XI_GenTL_Image_Format_xiMono8MS71": c_uint(
        0x80710008
    ),  # Pixel format 8bit TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono10MS71": c_uint(
        0x8071000A
    ),  # Pixel format 10bit TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono10pMS71": c_uint(
        0x8171000A
    ),  # Pixel format 10bit packed TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono12MS71": c_uint(
        0x8071000C
    ),  # Pixel format 12bit TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono12pMS71": c_uint(
        0x8171000C
    ),  # Pixel format 12bit packed TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono16MS71": c_uint(
        0x80710010
    ),  # Pixel format 16bit TOF (A+B+C)
    "XI_GenTL_Image_Format_xiBayerBG10LS37": c_uint(0x8037010A),  # Pixel format 10bit
    "XI_GenTL_Image_Format_xiBayerBG10pLS37": c_uint(
        0x8137010A
    ),  # Pixel format 10bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerBG12LS37": c_uint(0x8037010C),  # Pixel format 12bit
    "XI_GenTL_Image_Format_xiBayerBG12pLS37": c_uint(
        0x8137010C
    ),  # Pixel format 12bit packed (PFNC)
    "XI_GenTL_Image_Format_xiBayerBG14LS37": c_uint(0x8037010E),  # Pixel format 14bit
    "XI_GenTL_Image_Format_xiBayerBG14pLS37": c_uint(
        0x8137010E
    ),  # Pixel format 14bit packed (PFNC)
    ...
    "XI_GenTL_Image_Format_xiMono14MS71": c_uint(
        0x8071000E
    ),  # Pixel format 14bit TOF (A+B+C)
    "XI_GenTL_Image_Format_xiMono14pMS71": c_uint(
        0x8171000E
    ),  # Pixel format 14bit packed TOF (A+B+C)
    "XI_GenTL_Image_Format_xiToF10AnB": c_uint(
        0x80AB0F0A
    ),  # Pixel format 10bit ToF A and B (A in 2 bytes, B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF14AnB": c_uint(
        0x80AB0F0E
    ),  # Pixel format 14bit ToF A and B (A in 2 bytes, B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF16AnB": c_uint(
        0x80AB0F10
    ),  # Pixel format 16bit ToF A and B (A in 2 bytes, B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF10Aon": c_uint(
        0x80A20F0A
    ),  # Pixel format 10bit ToF A only (A in 2 bytes)
    "XI_GenTL_Image_Format_xiToF10ApB": c_uint(
        0x80A10F0A
    ),  # Pixel format 10bit ToF A + B (A+B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF10Bon": c_uint(
        0x80A30F0A
    ),  # Pixel format 10bit ToF B only (B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF10sAmB": c_uint(
        0x80A05F0A
    ),  # Pixel format 10bit signed ToF A - B (A-B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF14Aon": c_uint(
        0x80A20F0E
    ),  # Pixel format 14bit ToF A only (A in 2 bytes)
    "XI_GenTL_Image_Format_xiToF14ApB": c_uint(
        0x80A10F0E
    ),  # Pixel format 14bit ToF A + B (A+B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF14Bon": c_uint(
        0x80A30F0E
    ),  # Pixel format 14bit ToF B only (B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF14sAmB": c_uint(
        0x80A05F0E
    ),  # Pixel format 14bit signed ToF A - B (A-B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF16Aon": c_uint(
        0x80A20F10
    ),  # Pixel format 16bit ToF A only (A in 2 bytes)
    "XI_GenTL_Image_Format_xiToF16ApB": c_uint(
        0x80A10F10
    ),  # Pixel format 16bit ToF A + B (A+B in 2 bytes)
    "XI_GenTL_Image_Format_xiToF16Bon": c_uint(
        0x80A30F10
    ),  # Pixel format 16bit ToF B only (B in 2 bytes)
}

# Parameters

XI_PRM_EXPOSURE = "exposure"  # Exposure time in microseconds
XI_PRM_EXPOSURE_TIME_SELECTOR = (
    "exposure_time_selector"  # Selector for Exposure parameter
)
XI_PRM_EXPOSURE_BURST_COUNT = (
    "exposure_burst_count"  # Sets the number of times of exposure in one frame.
)
...
XI_PRM_SENS_DEFECTS_CORR = (
    "bpc"  # Correction of sensor defects (pixels, columns, rows) enable/disable
)
XI_PRM_AUTO_WB = "auto_wb"  # Automatic white balance
XI_PRM_MANUAL_WB = (
    "manual_wb"  # Calculates White Balance(xiGetImage function must be called)
)
...
XI_PRM_OFFSET_X = (
    "offsetX"  # Horizontal offset from the origin to the area of interest (in pixels).
)
XI_PRM_OFFSET_Y = (
    "offsetY"  # Vertical offset from the origin to the area of interest (in pixels).
)
XI_PRM_WIDTH_TOTAL = (
    "width_total"  # Total width of the Image provided by the device (in pixels).
)
XI_PRM_HEIGHT_TOTAL = (
    "height_total"  # Total height of the Image provided by the device (in pixels).
)
...
XI_PRM_REGION_MODE = (
    "region_mode"  # Activates/deactivates Region selected by Region Selector
)
...
XI_PRM_INTERLINE_EXPOSURE_MODE = (
    "interline_exposure_mode"  # Selector for Exposure parameter
)
XI_PRM_FFC = "ffc"  # Image flat field correction
XI_PRM_FFC_FLAT_FIELD_FILE_NAME = (
    "ffc_flat_field_file_name"  # Set name of file to be applied for FFC processor.
)
XI_PRM_FFC_DARK_FIELD_FILE_NAME = (
    "ffc_dark_field_file_name"  # Set name of file to be applied for FFC processor.
)
XI_PRM_TOF_READOUT_MODE = "tof_readout_mode"  # Sets ToF Readout Mode
XI_PRM_TOF_MODULATION_FREQUENCY = (
    "tof_modulation_frequency"  # Sets ToF Modulation Frequency in MHz
)
XI_PRM_TOF_MULTIPLE_PHASES_IN_BUFFER = (
    "tof_multiple_phases_in_buffer"  # is multiple ToF phases concatenated in buffer
)
XI_PRM_TOF_PHASES_COUNT = (
    "tof_phases_count"  # Sets the number of tof phases. E.g. 4 for four phases.
)
XI_PRM_TOF_PHASE_ANGLE = (
    "tof_phase_angle"  # Sets Illumination angle for selected ToF phase
)
...
XI_PRM_BINNING_VERTICAL_MODE = (
    "binning_vertical_mode"  # Sets the mode to use to combine vertical pixel together.
)
...
XI_PRM_BINNING_HORIZONTAL_PATTERN = (
    "binning_horizontal_pattern"  # Binning horizontal pattern type.
)
XI_PRM_BINNING_VERTICAL_PATTERN = (
    "binning_vertical_pattern"  # Binning vertical pattern type.
)
...
XI_PRM_DECIMATION_HORIZONTAL_PATTERN = (
    "decimation_horizontal_pattern"  # Decimation horizontal pattern type.
)
XI_PRM_DECIMATION_VERTICAL_PATTERN = (
    "decimation_vertical_pattern"  # Decimation vertical pattern type.
)
XI_PRM_EXP_PRIORITY = (
...
XI_PRM_IMAGE_DATA_BIT_DEPTH = (
    "image_data_bit_depth"  # bit depth of data returned by function xiGetImage
)
...
XI_PRM_TEMP_SELECTOR = (
    "temp_selector"  # Selector of mechanical point where thermometer is located.
)
...
XI_PRM_HOUS_BACK_SIDE_TEMP = (
    "hous_back_side_temp"  # Camera housing back side temperature
)
...
XI_PRM_GPI_LEVEL_AT_IMAGE_EXP_START = (
    "gpi_level_at_image_exp_start"  # GPI Level at image exposure start
)
XI_PRM_GPI_LEVEL_AT_IMAGE_EXP_END = (
    "gpi_level_at_image_exp_end"  # GPI Level at image exposure end
)
XI_PRM_GPI_LEVEL_AT_IMAGE_READOUT_END = (
    "gpi_level_at_image_readout_end"  # GPI Level at image readout end
)
XI_PRM_GPO_SELECTOR = "gpo_selector"  # Selects GPO
XI_PRM_GPO_MODE = "gpo_mode"  # Defines GPO functionality
XI_PRM_METADATA_SAMPLING_MODE = (
    "metadata_sampling_mode"  # Defines metadata sampling mode
)
...
XI_PRM_DEBOUNCE_POL = (
    "dbnc_pol"  # Debounce polarity (pol = 1 t0 - falling edge, t1 - rising edge)
)
...
XI_PRM_LENS_APERTURE_INDEX = (
    "lens_aperture_index"  # Current aperture index as reported by lens.
)
...
XI_PRM_LENS_FOCUS_DISTANCE = (
    "lens_focus_distance"  # (Planned feature Issue#6958). Lens focus distance in cm.
)
...
XI_PRM_DEVICE_LOCATION_PATH = (
    "device_loc_path"  # Represents the location of the device in the device tree.
)
...
XI_PRM_TRANSPORT_PIXEL_FORMAT = (
    "transport_pixel_format"  # Current format of pixels on transport layer.
)
XI_PRM_TRANSPORT_DATA_TARGET = (
    "transport_data_target"  # Target selector for data - CPU RAM or GPU RAM
)
...
XI_PRM_AVAILABLE_BANDWIDTH = (
    "available_bandwidth"  # Measure available interface bandwidth (in Megabits)
)
XI_PRM_BUFFER_POLICY = "buffer_policy"  # Data move policy
XI_PRM_LUT_EN = "LUTEnable"  # Activates LUT.
XI_PRM_LUT_INDEX = (
    "LUTIndex"  # Control the index (offset) of the coefficient to access in the LUT.
)
...
XI_PRM_ACQ_TRANSPORT_PACKET_SIZE = (
    "acq_transport_packet_size"  # Acquisition transport packet size in bytes
)
...
XI_PRM_CONCAT_IMG_MODE = (
    "concat_img_mode"  # Enable/disable the Concatenated Images in One Buffer feature
)
XI_PRM_CONCAT_IMG_COUNT = (
    "concat_img_count"  # Number of Concatenated Images in One Buffer
)
...
XI_PRM_COLUMN_BLACK_OFFSET_CORRECTION = (
    "column_black_offset_correction"  # Correction of column black offset
)
XI_PRM_ROW_BLACK_OFFSET_CORRECTION = (
    "row_black_offset_correction"  # Correction of row black offset
)
XI_PRM_IMAGE_CORRECTION_SELECTOR = (
    "image_correction_selector"  # Select image correction function
)
XI_PRM_IMAGE_CORRECTION_VALUE = (
    "image_correction_value"  # Select image correction selected function value
)
...
XI_PRM_KNEEPOINT1 = (
    "hdr_kneepoint1"  # value of first kneepoint (% of sensor saturation)
)
XI_PRM_KNEEPOINT2 = (
    "hdr_kneepoint2"  # value of second kneepoint (% of sensor saturation)
)
XI_PRM_TRANS_DATA_BLACK_LEVEL_OVR = (
    "trans_data_black_level_ovr"  # Overwrites black level comming from transport data.
)
XI_PRM_TRANS_DATA_BLACK_LEVEL_OVR_EN = (
    "trans_data_black_level_ovr_en"  # Enables/disables black level overwrite.
)
...
XI_PRM_COMPRESSION_REGION_SELECTOR = (
    "compression_region_selector"  # Sets Compression Region Selector
)
XI_PRM_COMPRESSION_REGION_START = (
    "compression_region_start"  # Sets Compression Region Start
)
XI_PRM_COMPRESSION_REGION_GAIN = (
    "compression_region_gain"  # Sets Compression Region Gain
)
XI_PRM_VERSION_SELECTOR = (
    "version_selector"  # Selects module/unit, which version we get.
)
XI_PRM_VERSION = (
    "version"  # Returns version of selected module/unit(XI_PRM_VERSION_SELECTOR).
)
...
XI_PRM_FPGA1_VERSION = (
    "version_fpga1"  # Returns version of FPGA firmware currently running.
)
...
XI_PRM_AUTO_BANDWIDTH_CALCULATION = (
    "auto_bandwidth_calculation"  # Automatic bandwidth calculation,
)
...
XI_PRM_CAM_ENUM_GOLDEN_ENABLED = (
    "cam_enum_golden_enabled"  # Enable enumeration of golden devices
)
XI_PRM_RESET_USB_IF_BOOTLOADER = (
    "reset_usb_if_bootloader"  # Resets USB device if started as bootloader
)
XI_PRM_CAM_SIMULATORS_COUNT = (
    "cam_simulators_count"  # Number of camera simulators to be available.
)
...
XI_PRM_DEVICE_UNIT_REGISTER_ENUM_VALUE_COUNT = (
    "device_unit_register_enum_value_count"  # Read register enumerated values count.
)
...
XI_PRM_DEVICE_UNIT_REGISTER_ENUM_VALUE = (
    "device_unit_register_enum_value"  # Read register enumerated value.
)
...
XI_PRM_DP_PARAM_SELECTOR = (
    "dp_param_selector"  # Data Pipe Processor parameter Selector.
)
XI_PRM_DP_PARAM_VALUE = "dp_param_value"  # Data Pipe processor parameter value
XI_PRM_GENTL_DATASTREAM_ENABLED = (
    "gentl_stream_en"  # Enable or disable low level streaming via GenTL.
)
XI_PRM_GENTL_DATASTREAM_CONTEXT = (
    "gentl_stream_context"  # Get GenTL stream context pointer for low level streaming
)
XI_PRM_USER_SET_SELECTOR = (
    "user_set_selector"  # Selects the feature User Set to load, save or configure.
)
...
class XI_IMG(Structure):
    ...
    _fields_ = [
        (
            "size",
            DWORD,
        ),  # Size of current structure on application side. When xiGetImage is called and size>=SIZE_XI_IMG_V2 then GPI_level, tsSec and tsUSec are filled.
        ...
        (
            "nframe",
            DWORD,
        ),  # Frame number. On some cameras it is reset by exposure, gain, downsampling change, auto exposure (AEAG).
        ("tsSec", DWORD),  # Seconds part of image timestamp (see Note3).
        (
            "tsUSec",
            DWORD,
        ),  # Micro-seconds part image timestamp (see Note3). Range 0-999999 us.
        (
            "GPI_level",
            DWORD,
        ),  # Levels of digital inputs/outputs of the camera at time of exposure start/end (sample time and bits are specific for each camera model)
        (
            "black_level",
            DWORD,
        ),  # Black level of image (ONLY for MONO and RAW formats). (see Note4)
        (
            "padding_x",
            DWORD,
        ...
        (
            "exposure_time_us",
            DWORD,
        ),  # Exposure time of this image in microseconds. (see Note5)
        ("gain_db", FLOAT),  # Gain used for this image in deci-bells. (see Note6)
        (
            "acq_nframe",
            DWORD,
        ),  # Frame number. Reset only by acquisition start. NOT reset by change of exposure, gain, downsampling, auto exposure (AEAG).
        ("image_user_data", DWORD),  # (see Note7)
        (
            "exposure_sub_times_us",
            DWORD * 5,
        ),  # Array with five sub exposures times in microseconds used by XI_TRG_SEL_MULTIPLE_EXPOSURES or hardware controlled HDR        #(see Note8)
        ...
        (
            "lg_black_level",
            DWORD,
        ),  # In case of multi gain channel readout, the black level low gain channel
        (
            "hg_black_level",
            DWORD,
        ),  # In case of multi gain channel readout, the black level high gain channel
        (
            "lg_range",
            DWORD,
        ),  # In case of multi gain channel readout, the valid range of low gain channel
        (
            "hg_range",
            DWORD,
        ),  # In case of multi gain channel readout, the valid range of high gain channel
        (
            "gain_ratio",
            FLOAT,
        ),  # Gain ratio for dual-channel modes (high_gain_channel/low_gain_channel). Unitless.
        ...
        (
            "tof_phases_count",
            DWORD,
        ),  # Number of phases of ToF sensor. E.g. 4 for four phases.
        ...
        (
            "sequence_type",
            c_uint,
        ),  # Type of image sequence (e.g. seq_disabled, seq_multiple_exposures)
        (
            "sequence_image_id",
            DWORD,
        ),  # Current image in sequence. Starting from 1. Zero if sequence is disabled.
        (
            "sequence_length",
            DWORD,
        ),  # Count of images in sequence. Zero for sequence disabled.
    ]
